...
int main(void){
    ...
    stack_t s={.ss_sp=alternate,.ss_size=sizeof alternate};assert(!sigaltstack(&s,0));struct sigaction sa={.sa_handler=handler,.sa_flags=SA_ONSTACK};sigemptyset(&sa.sa_mask);assert(!sigaction(SIGUSR1,&sa,0));assert(!raise(SIGUSR1));assert(handled==1);assert(!sigaltstack(0,&s)&&!(s.ss_flags&SS_ONSTACK));s.ss_flags=SS_DISABLE;assert(!sigaltstack(&s,0));
